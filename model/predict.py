...
class Predict():
    ...
    def greedy_search(self,
        ...
        decoder_input_t = torch.tensor([self.vocab.SOS])
        ...
        while int(decoder_input_t.item()) != (self.vocab.EOS) \
            ...
            context_vector, attention_weights = self.model.attention(decoder_states, 
                                                                     encoder_output, 
                                                                     x_padding_masks)
            p_vocab, decoder_states = self.model.decoder(decoder_input_t.view(1,1), 
                                                         decoder_states, 
                                                         encoder_output, 
                                                         context_vector)
...
#     @timer('best k')
    def best_k(self, beam, k, encoder_output, x_padding_masks):
        ...
        decoder_input_t = torch.tensor(beam.tokens[-1]).view(1,1)
        decoder_input_t = decoder_input_t.to(self.DEVICE)

        # Get context vector from attention network.
        context_vector, attention_weights = self.model.attention(beam.decoder_states, 
                                                                 encoder_output, 
                                                                 x_padding_masks)
        ...
        log_probs = torch.log(p_vocab.squeeze(0))
