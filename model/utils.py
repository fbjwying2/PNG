...
def outputids2words(id_list, source_oovs, vocab):
    ...
    for index in id_list:
        if index < len(vocab.index2word):
            words.append(vocab.index2word[index])
        else:               
            oov_index = index - len(vocab)
            if oov_index in source_oovs:
                words.append(source_oovs[oov_index])
            else:
                words.append(vocab.index2word[vocab.UNK])
                print("ERROR OOV index:",index)   
...
def source2ids(source_words, vocab):
    ...
    oov_index = len(vocab)
    for source_word_index, word in enumerate(source_words):
        word_index = vocab[word]
        if word_index != unk_id:
